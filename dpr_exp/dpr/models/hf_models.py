...
def get_lstm_biencoder_components(args, inference_only: bool = False, **kwargs):
    dropout = args.dropout if hasattr(args, 'dropout') else 0.0
    question_encoder = BiLSTMEncoder.init_encoder(
        args.pretrained_model_cfg, projection_dim=args.projection_dim, dropout=dropout, **kwargs
    )
    ctx_encoder = BiLSTMEncoder.init_encoder(
        args.pretrained_model_cfg, projection_dim=args.projection_dim, dropout=dropout, **kwargs
    )
    ...
    if inference_only:
        ...
        parameters = [p for p in biencoder.parameters() if p.requires_grad]
        optimizer = torch.optim.Adam(parameters, args.learning_rate, weight_decay=args.weight_decay)
...
class BiLSTMEncoder(nn.Module):
    ...
    def __init__(self, config, project_dim: int = 0):
        ...
        self.encoder = nn.LSTM(
            config.hidden_size, config.hidden_size, num_layers=1, bidirectional=True, batch_first=True
        )

    @classmethod
    def init_encoder(cls, cfg_name: str, projection_dim: int = 0, dropout: float = 0.1, **kwargs):
        cfg = RobertaConfig.from_pretrained(cfg_name if cfg_name else 'roberta-base')
        if dropout != 0:
            cfg.attention_probs_dropout_prob = dropout
            cfg.hidden_dropout_prob = dropout
    ...
    def forward(self, input_ids: T, token_type_ids: T, attention_mask: T) -> Tuple[T, ...]:
        ...
        # pooled_output[attention_mask == 0] = -1e9
        # pooled_output = torch.max(pooled_output, 1)[0]
        # assert pooled_output.ndimension() == 2
