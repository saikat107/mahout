...
def add_training_params(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--train_file", default=None, nargs='+', help="Train files")
    parser.add_argument("--dev_file", default=None, nargs='+', help="Validation files")
...
def set_encoder_params_from_state(state, args, quiet=False):
    if not state:
        return
    params_to_save = get_encoder_checkpoint_params_names()
