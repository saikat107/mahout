...
def read_data_from_json_files(
    ...
    for i, path in enumerate(paths):
        if dataset.startswith('csnet'):
            logger.info('Parsing %s data from %s' % (dataset, path))
            reader = open(path, encoding='utf-8')
            data = json.load(reader)
            processed_data = []
            for d in data:
                for _ in range(upsample_rates[i]):
...
class ShardedDataIterator(object):
    ...
    def __init__(
            self, data: list, shard_id: int = 0, num_shards: int = 1, batch_size: int = 1, shuffle=True,
