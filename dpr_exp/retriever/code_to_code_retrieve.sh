...
if [[ $# -lt 3 ]]; then
    ...
    echo "bash retrieve.sh <gpuids> <dataset> <index_lang> [<model_type>, default=codebert-base]";
    exit;
fi
...
script="${CODE_BASE_DIR}/retriever/source/code_to_code_retrieve.py";
BATCH_SIZE=512;

RESULT_BASE_DIR="/home/${USER_NAME}/workspace/projects/RetrievalBasedCodeTranslation";

OUT_DIR="${RESULT_BASE_DIR}/retrieved_docs/${DATASET_NAME}";
mkdir -p $OUT_DIR;
...
for SPLIT in train valid; do
    ...
    OUT_FILE="${ODIR}/code_to_code_top${TOP_K}_${INDEX_LANG}.json";
    LOG_FILE="${ODIR}/top${TOP_K}_${INDEX_LANG}.log";
    if [[ ! -f $OUT_FILE ]]; then
        python ${script} \
            --fp16 \
            --no_eval \
