...
class BiEncoderTrainer(object):
    ...
    def get_data_iterator(
            ...
            dataset_name: str = "csnet-java"
    ) -> ShardedDataIterator:
        data = read_data_from_json_files(paths=data_files, upsample_rates=upsample_rates, dataset=dataset_name)
    ...
    def validate_average_rank(self) -> float:
        ...
        data_iterator = self.get_data_iterator(
            args.dev_file, args.dev_batch_size, shuffle=False, dataset_name=args.dataset
        )
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_data()):
            ...
            for j, batch_start in enumerate(range(0, bsz, sub_batch_size)):
                ...
                q_ids, q_segments = (biencoder_input.question_ids, biencoder_input.question_segments) \
            ...
            if (i + 1) % log_result_step == 0:
                logger.info(
                    'Av.rank validation: step %d, computed ctx_vectors %d, q_vectors %d', i,
        ...
        if distributed_factor > 1:
            # each node calcuated its own rank, exchange the information between node and calculate the "global"
            # average rank
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default='csnet', help="Name of the dataset")
